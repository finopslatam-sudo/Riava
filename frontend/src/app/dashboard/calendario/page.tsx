...
// End-time options go up to 21:30
const END_TIME_OPTIONS: string[] = []
for (let h = 7; h <= 21; h++) {
  END_TIME_OPTIONS.push(`${String(h).padStart(2, '0')}:30`)
  if (h < 21) END_TIME_OPTIONS.push(`${String(h + 1).padStart(2, '0')}:00`)
}
END_TIME_OPTIONS.push('21:30')
...
function generateSlotsInRange(startTime: string, endTime: string): Array<{ startTime: string; endTime: string }> {
...
function getSaturdaysInMonth(year: number, month: number): string[] {
  ...
  for (let d = 1; d <= days; d++) {
    if (new Date(year, month, d).getDay() === 6) {
      saturdays.push(isoDate(year, month, d))
    }
  }
  return saturdays
}
...
export default function CalendarioPage() {
  ...
  // Repeat controls — independent loading per action
  const [repeatMonFriLoading, setRepeatMonFriLoading] = useState(false)
  ...
  // Compute slots that would be created for the chosen range
  ...
  const repeatMonFri = async () => {
    ...
    await fetchSlots()
    setRepeatMonFriLoading(false)
    setRepeatMsg(`✓ ${created} horario${created !== 1 ? 's' : ''} creado${created !== 1 ? 's' : ''} Lun–Vie`)
  ...
  const repeatSaturdays = async () => {
    ...
    setRepeatMsg(`✓ ${created} horario${created !== 1 ? 's' : ''} creado${created !== 1 ? 's' : ''} en Sábados del mes`)
    setTimeout(() => setRepeatMsg(''), 4000)
  }
  ...
  const anyRepeatLoading = repeatMonFriLoading || repeatSatLoading
              ...
              <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
                ...
                <div>
                  ...
                  {previewSlots.length > 0 ? (
                    ...
                    <p className="text-xs mb-2" style={{ color: 'rgba(240,0,255,0.6)' }}>
                      El horario de fin debe ser al menos 30 min después del inicio.
                    </p>
                  )}
                  ...
                  <button
                    onClick={addSlots}
                    disabled={addLoading || previewSlots.length === 0}
                ...
                {hasRepeatableSlots && (
                  <div className="flex flex-col gap-2">
                    <p className="text-xs font-semibold tracking-widest uppercase" style={{ color: 'rgba(0,229,255,0.4)' }}>
                      Repetir horarios
                    </p>

                    <button
                      onClick={repeatMonFri}
                      disabled={anyRepeatLoading}
                      className="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-semibold transition-all disabled:opacity-50"
                    ...
                    >
                      ...
                      {repeatMonFriLoading ? 'Creando...' : 'Repetir Lun–Vie esta semana'}
                    </button>

                    <button
                      onClick={repeatSaturdays}
                      disabled={anyRepeatLoading}
                      className="w-full flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-semibold transition-all disabled:opacity-50"
                      style={{ background: 'rgba(0,229,255,0.04)', border: '1px solid rgba(0,229,255,0.15)', color: 'rgba(0,229,255,0.7)' }}
                      onMouseEnter={e => { if (!anyRepeatLoading) e.currentTarget.style.background = 'rgba(0,229,255,0.09)' }}
                    ...
                    >
                      ...
                      {repeatSatLoading ? 'Creando...' : `Repetir todos los Sábados de ${MONTHS_ES[viewMonth]}`}
                    </button>

                    {repeatMsg && (
                      <p className="text-xs text-center" style={{ color: repeatMsg.startsWith('✓') ? '#00e5ff' : '#f000ff' }}>
                        {repeatMsg}
                      </p>
                    )}
                  </div>
