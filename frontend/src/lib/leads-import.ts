...
export async function importLeadFromWebhook(pageAccessToken: string, formId: string, leadgenId: string): Promise<{ imported: boolean }> {
  const formRes = await fetch(
    `https://graph.facebook.com/v19.0/${formId}?` +
    new URLSearchParams({ fields: 'name,questions', access_token: pageAccessToken })
  )
  if (!formRes.ok) return { imported: false }
  const form = await formRes.json() as { name: string; questions?: FormQuestion[] }
  ...
  const leadRes = await fetch(
    ...
    new URLSearchParams({ fields: 'id,created_time,campaign_name,field_data', access_token: pageAccessToken })
  )
  if (!leadRes.ok) return { imported: false }
  const ml = await leadRes.json() as { id: string; created_time: string; campaign_name?: string; field_data: FieldData[] }

  const email = field(ml.field_data, 'email', 'correo_electronico', 'correo', 'e-mail')
  if (!email) return { imported: false }

  const existingLeads = await getAllLeads()
  const existing = existingLeads.find(l => l.email.toLowerCase() === email.toLowerCase())
  if (existing) {
    if (existing.created_at !== ml.created_time) await updateLead(existing.id, { created_at: ml.created_time })
